...
def save_admission_from_form(data, reg_no, status='Submitted'):
    ...
    # Keep student registration profile fields in sync
    from accounts.models import Student
    Student.objects.filter(registration_no=reg_no).update(
        full_name=defaults.get('full_name', ''),
        mobile=defaults.get('mobile', ''),
        email=defaults.get('email', ''),
        aadhaar=defaults.get('aadhaar', ''),
        program_type=defaults.get('program_type', ''),
        course_name=defaults.get('course_name', '')
    )
